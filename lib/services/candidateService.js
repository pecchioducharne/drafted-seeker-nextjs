...
export async function fetchCandidates(filters = {}, fetchLimit = 2000, forceRefresh = false) {
  ...
  try {
    ...
    // Apply Firestore-level filters (these need indexes)
    if (filters.hasVideo1) {
      queryConstraints.push(where('video1', '!=', null));
    }
    ...
    if (filters.graduationYear) {
      queryConstraints.push(where('graduationYear', '==', filters.graduationYear));
    }

    // Order by createdAt desc for recent-first
    if (queryConstraints.length > 0) {
      // When we have a where clause, we need to order by the same field first (Firestore requirement)
      if (filters.hasVideo1) {
        queryConstraints.push(orderBy('video1'));
      }
