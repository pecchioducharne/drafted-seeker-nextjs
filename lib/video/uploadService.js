 ...
 * @param {boolean} enableTranscription - Whether to trigger transcription (default: true)
 * @returns {Promise<{url: string, saved: boolean, transcriptionStarted: boolean}>}
 */
export async function uploadVideoAndSave(blob, userEmail, videoNumber, onProgress, enableTranscription = true) {
  // Upload to storage
  ...
  try {
    ...
    // Trigger transcription asynchronously (don't block on completion)
    let transcriptionStarted = false;
    if (enableTranscription) {
      try {
        // Dynamic import to avoid circular dependencies
        const { default: transcribeAudio } = await import('../services/TranscriptionService');
        
        // Start transcription in background - don't await
        transcribeAudio(blob, userEmail, videoNumber)
          .then(() => {
            console.log(`✅ Transcription completed for video ${videoNumber}`);
          })
          .catch((error) => {
            console.error(`❌ Transcription failed for video ${videoNumber}:`, error);
          });
        
        transcriptionStarted = true;
        console.log(`🎤 Transcription started for video ${videoNumber}`);
...
export async function uploadScreenRecording(blob, userEmail, onProgress) {
  return uploadVideoAndSave(blob, userEmail, 3, onProgress, true);
}
